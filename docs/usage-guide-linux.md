...
- Linux（Ubuntu 20.04+ / Debian 11+ / Fedora 38+ / Arch 等主流发行版）
- Go 1.22+（仅从源码构建时需要）
...
# 永久生效（Bash）
echo 'export PATH="$PATH:$HOME/tools/aict"' >> ~/.bashrc
source ~/.bashrc
...
# 写入 shell 配置文件以永久生效
echo 'export AICT_CLAUDE_API_KEY="sk-ant-xxxxx"' >> ~/.bashrc
```
...
cp dist/aict-linux-amd64 ~/my-aict/aict
...
```text
> 找出 /var/log 目录下所有超过 100MB 的日志文件
```
...
2. 启动服务：`systemctl start ollama` 或直接运行 `ollama serve`
3. 拉取模型：`ollama pull qwen2.5-coder:7b`
4. 运行 `aict init`，选择 `4) Ollama (本地)`
5. 模型名称填你已下载的模型，如 `qwen2.5-coder:7b`
...
可以。aict 是纯终端应用，支持任何 TTY 环境，包括 SSH、tmux、screen 等。

### Q: 如何设置为 systemd 服务？

aict 是交互式工具，不适合作为后台服务运行。它需要用户在终端中交互操作。
